defmodule RoboticaPlugins.Executor do
  use GenServer
  ...
  def execute(server, %RoboticaPlugins.Task{locations: locations, action: action}) do
  ...
  defp handle_execute(state, locations, action) do
    Enum.each(locations, fn location ->
      ...
      Enum.each(plugins, fn pid ->
        RoboticaPlugins.execute(pid, action)
      end)

      Enum.each(plugins, fn pid ->
        RoboticaPlugins.wait(pid)
      end)
    end)
